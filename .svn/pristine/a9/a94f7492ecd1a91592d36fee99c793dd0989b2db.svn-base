...
var localhostStr='http://101.200.188.251:8085/gyapp_api'
  //var localhostStr='http://10.0.0.9:8080/gyapp_api'
//交易查询
var month =''
var page='2'
//充值卡
...
const store = new Vuex.Store({
  ...
  state: {
    ...
    TransactionRecord:{
      ...
      pramater: {loginUser : this.loginUserId,flag : this.flag,date: '' ,month :'',page:'1',tradeType:''},
      result:[],
      income_money:'',
      pay_money:'',
      tradeId:[],
  ...
  actions: {
    ...
    getTransactionRecord (context) {
      ...
      context.state.TransactionRecord.pramater.month =year+'-'+month
      axios.post(context.state.TransactionRecord.url, qs.stringify(context.state.TransactionRecord.pramater), {emulateJSON: true}).then(
        function (res) {
          if(res.data.ret){
            ...
            console.log(  context.state.TransactionRecord.result)
    ...
    getCommissionRate (context){
      context.state.CommissionRate.pramater.loginUser = context.state.loginUserId
       axios.post(context.state.CommissionRate.url, qs.stringify(context.state.CommissionRate.pramater), {emulateJSON: true}).then(
          function(res){
            if(res.data.ret){
              context.state.CommissionRate.result=res.data
              console.log( res.data)
            }else{
              alert(res.data.tip)
            }
    ...
    getSettlementquery (context){
     ...
     context.state.Settlementquery.pramater.month =year+'-'+month
    ...
    getDetailInquirySummary (context){
          ...
          context.state.DetailInquirySummary.pramater.month =year+'-'+month
    ...
    getRequest:function(context){
       context.state.Request.pramater.loginUser = context.state.loginUserId
       context.state.Request.pramater.flag = context.state.flag
      axios.post(context.state.Request.url,qs.stringify(context.state.Request.pramater),{emulateJSON: true}).then(
        function(res){
           if(res.data){
             context.state.Request.result=res.data
               console.log( context.state.Request.result)
           }else{
